...
async def test_id_preservation_pattern_works(db_pool):
    """The 0008 migration backfilled parents/students/contacts into
    people preserving ids, and the route swaps in 0010-0012 trust that
    invariant. After PR-4 drops the legacy `parents` table we can't
    replay the original test (it inserted into parents, then asserted
    the same id appeared in people). This is the smaller invariant
    that's still verifiable: explicit ids round-trip through people
    cleanly, which is what the backfill relied on."""
    async with db_pool.acquire() as conn:
        person_id = await conn.fetchval(
